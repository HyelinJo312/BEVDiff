...
task-specific priors or ground-truth annotations for
conditioning~\citep{BEVDiff,LaneDiff}. In particular, BEVDiffuser constructs a
ground-truth object layout from object categories and 3D bounding boxes and
employs a conditioning mechanism specialized for this layout~\citep{BEVDiff}.
Although such conditioning provides strong task-aligned guidance, it introduces
additional annotation requirements and couples the diffusion model to 3D object
detection. Adapting the model to another downstream task may therefore require
redesigning both the conditioning signal and its injection mechanism.

Task dependency also arises from the training objective. Existing BEV diffusion
...
training to task-specific prediction heads and supervision. Although
...
approaches to a new task may therefore require modifying the prediction head and
training objectives, limiting the transferability of diffusion-based BEV
refinement across downstream tasks.

To address these limitations, we propose a task-agnostic semantic-guided
diffusion framework for BEV representation learning, as illustrated in
Figure~\ref{fig:overview}. Here, task-agnostic means that diffusion training
does not require task-specific annotations or downstream task supervision. A
frozen vision foundation model~\citep{SAM3} extracts multi-view semantic masks,
which are projected into the BEV space to construct a spatially aligned dense
semantic BEV prior. This prior captures both dynamic agents and static scene
elements and guides denoising through Semantic BEV Adaptive Modulation (SBAM).
We first pre-train the diffusion model using only a generative denoising
objective and then freeze it as a teacher. The denoised BEV representations are
subsequently transferred to a student BEV model through masked generative
distillation~\citep{MGD}. At inference time, the diffusion teacher and semantic 
branch are removed, introducing no additional inference cost.
...
\begin{itemize}
    ...
    \item Building on the plug-and-play paradigm of BEVDiffuser, we introduce
    masked generative BEV distillation to transfer semantic-guided denoised
    representations to a student model, achieving competitive performance
    against task-specific diffusion approaches without additional inference
    cost.
\end{itemize}
...
the BEV space and isused to learn task-agnostic scene representations rather than 
synthesize images.
