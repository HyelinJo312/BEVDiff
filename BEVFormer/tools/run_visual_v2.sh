...
# ---------- JSON paths ----------
OURS_JSON="../results/version2/stage2/DiffBEVFormer_tiny_seg_v5/val/Tue_Apr_28_13_42_34_2026/pts_bbox/results_nusc.json"
BASE_JSON="../results/version2/stage2/DiffBEVFormer_tiny_original_24epoch/val/Thu_Apr_23_12_52_35_2026/pts_bbox/results_nusc.json"
...
# + point_cloud_range). Leave empty to fall back to raw nuScenes annotations.
...
if [ -n "${GT_PKL}" ]; then
    if [ ! -f "${GT_PKL}" ]; then
        echo "[run_visual_v2] WARNING: GT_PKL='${GT_PKL}' not found; GT panels will use raw nuScenes annotations." >&2
    else
        GT_ARGS+=(--gt_pkl "${GT_PKL}")
        echo "[run_visual_v2] gt_pkl: ${GT_PKL} (training-time GT filtering enabled)"
    fi
fi
