...
class BEVDiffuser(nn.Module):
    ...
    def forward(self, x, img, img_metas, condition=None, grad_fn=None): 
        # DINO condition
        ...
        if self.denoise_timesteps > 0:   
            ...
            for _, t in enumerate(self.noise_scheduler.timesteps):
                ...
                noise_pred_uncond, noise_pred_cond = self.unet(x, t_batch, dino_uncond, **uncond)[0], self.unet(x, t_batch, dino_cond, **cond)[0]
        ...
        # else:
        # timestep = torch.tensor([100] * x.shape[0], device=x.device)  
        # multi_feat = self.unet(x, timestep, dino_cond, **cond)[1]    
        # return multi_feat