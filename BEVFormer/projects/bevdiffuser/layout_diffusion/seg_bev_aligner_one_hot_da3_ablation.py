...
class SegBEVEncoder(nn.Module):
    """
    A1 ablation (ControlNet-style input-concat): seg 는 UNet 입력단에서 ds=1 맵만 사용한다.
    원본은 멀티스케일 {1,2,4} 를 만들어 decoder FDN 에 주입했으나, 여기서는 ds=1 만 쓰므로
    down1/down2 (ds=2, ds=4 strided conv) 를 아예 제거한다.
    → 미사용 파라미터가 없어져 DDP 의 unused-parameter 에러가 근본적으로 사라진다.
    ...
    def __init__(self, in_channels, channel_mult=(1, 2, 4)):
        ...
        # ds=1: Spatial processing (해상도/채널 유지) — 유일하게 사용되는 스케일
        self.down0 = nn.Sequential(
            nn.Conv2d(c1, c1, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
        )
        # NOTE: down1(ds=2), down2(ds=4) 는 input-concat ablation 에서 미사용이라 제거.
