...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, cond_kwargs=None, seg_cond=None):
        extra_output = None
        for layer in self:
...
class GatedFDN(nn.Module):
    """Channel-wise gated FDN: 네트워크가 채널별로 FDN 영향력을 학습."""
...
class AdvancedGatedFDN(nn.Module):
    """CBAM-style Spatial-Channel Decoupled Gating for FDN.
    gate = g_c(channel) * g_s(spatial)  →  (B, C, H, W)
    out  = GroupNorm(x) * (1 + gate * γ) + gate * β
    """
    def __init__(self, norm_channels, cond_channels, reduction=8):
        super().__init__()

        bottleneck = max(norm_channels // reduction, 32)  # 최소 32로 제한
        
        self.param_free_norm = normalization(norm_channels)
        self.conv_gamma = nn.Conv2d(cond_channels, norm_channels, kernel_size=3, padding=1)
        self.conv_beta  = nn.Conv2d(cond_channels, norm_channels, kernel_size=3, padding=1)

        # === (a) Channel Gate: CBAM Channel Attention ===
        # Global AvgPool + Global MaxPool → bottleneck MLP → (B, C, 1, 1)
        self.channel_avg_pool = nn.AdaptiveAvgPool2d(1)
        self.channel_max_pool = nn.AdaptiveMaxPool2d(1)
        self.channel_fc = nn.Sequential(
            nn.Linear(norm_channels, bottleneck, bias=True),
            nn.ReLU(inplace=True),
            nn.Linear(bottleneck, norm_channels, bias=True),
        )

        # === (b) Spatial Gate: CBAM Spatial Attention ===
        self.spatial_conv = nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=True)

        # self._init_conservative()

    def _init_conservative(self):
        """Gate를 닫힌 상태로 초기화하여 원본 BEV 피처를 보존."""
        # Channel gate: FC 출력 bias=-2.0 → sigmoid(-2)≈0.119
        nn.init.zeros_(self.channel_fc[2].weight)
        nn.init.constant_(self.channel_fc[2].bias, -1.0)
        # Spatial gate: conv 출력 bias=-2.0 → sigmoid(-2)≈0.119
        nn.init.zeros_(self.spatial_conv.weight)
        nn.init.constant_(self.spatial_conv.bias, -1.0)

    def forward(self, x, seg_cond):
        B, C, H, W = x.shape

        # --- Step 1: Normalization & Affine ---
        x_norm = self.param_free_norm(x)               # (B, C, H, W)
        gamma = self.conv_gamma(seg_cond)               # (B, C, H, W)
        beta  = self.conv_beta(seg_cond)                # (B, C, H, W)

        # --- Step 2a: Channel Gate (CBAM Channel Attention, shared MLP) ---
        x_avg = self.channel_avg_pool(x).squeeze(-1).squeeze(-1)  # (B, C)
        x_max = self.channel_max_pool(x).squeeze(-1).squeeze(-1)  # (B, C)
        g_c = torch.sigmoid(self.channel_fc(x_avg) + self.channel_fc(x_max))  # (B, C)
        g_c = g_c.unsqueeze(-1).unsqueeze(-1)            # (B, C, 1, 1)

        # --- Step 2b: Spatial Gate (CBAM Spatial Attention) ---
        x_mean = x.mean(dim=1, keepdim=True)             # (B, 1, H, W)
        x_max_s = x.amax(dim=1, keepdim=True)            # (B, 1, H, W)
        spatial_in = torch.cat([x_mean, x_max_s], dim=1) # (B, 2, H, W)
        g_s = torch.sigmoid(self.spatial_conv(spatial_in)) # (B, 1, H, W)

        # --- Step 3: Compose & Apply ---
        gate = g_c * g_s
        out = x_norm * (1 + gate * gamma) + gate * beta  # (B, C, H, W)
        return out


class AdvancedGatedFDNv2(nn.Module):
    """Asymmetric Dual-Gate FDN for BEV 3D Geometry Preservation.
    gate = g_sem * g_bev   →  (B, C, H, W)
    out  = GroupNorm(x) * (1 + gate * γ) + gate * β
    """
    def __init__(self, norm_channels, cond_channels, reduction=16):
        super().__init__()

        bottleneck = max(norm_channels // reduction, 32)
        ...
        # === (a) Semantic Gate: from seg_cond ===
        # "어떤 채널/위치에 전달할 만한 semantic 신호가 존재하는가?"
        self.sem_channel_fc = nn.Sequential(
            nn.Linear(cond_channels, bottleneck, bias=True),   # C_s → bottleneck
            nn.ReLU(inplace=True),
            nn.Linear(bottleneck, norm_channels, bias=True),   # bottleneck → C
        )
        self.sem_spatial_conv = nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=True)

        # === (b) BEV Gate: from x ===
        # "이 채널/위치를 modulate해도 3D geometry 정보가 손실되지 않는가?"
        self.bev_channel_fc = nn.Sequential(
            nn.Linear(norm_channels, bottleneck, bias=True),   # C → bottleneck
            nn.ReLU(inplace=True),
            nn.Linear(bottleneck, norm_channels, bias=True),   # bottleneck → C
        )
        self.bev_spatial_conv = nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=True)

        self._init_conservative()

    def _init_conservative(self):
        """BEV gate를 닫힌 상태로 초기화 → 3D geometry 보호 우선.
        Semantic gate는 default init → seg_cond 신호를 바로 활용 가능."""
        # BEV gate: 초기에 닫혀있음 → 학습하며 안전한 채널/위치만 열림
        nn.init.zeros_(self.bev_channel_fc[2].weight)
        nn.init.constant_(self.bev_channel_fc[2].bias, -1.0)
        nn.init.zeros_(self.bev_spatial_conv.weight)
        nn.init.constant_(self.bev_spatial_conv.bias, -1.0)

    def forward(self, x, seg_cond):
        """
        Args:
            x:        (B, C, H, W)   BEV features (3D geometry 인코딩 포함)
            seg_cond: (B, C_s, H, W) BEV-projected segmentation condition
        Returns:
            out:      (B, C, H, W)   선택적으로 conditioning된 BEV features
        """
        B, C, H, W = x.shape

        # --- Step 1: Normalization & Affine (from seg_cond only) ---
        x_norm = self.param_free_norm(x)                 # (B, C, H, W)
        gamma = self.conv_gamma(seg_cond)                 # (B, C, H, W)
        beta  = self.conv_beta(seg_cond)                  # (B, C, H, W)

        # --- Step 2a: Semantic Gate (from seg_cond) ---
        # Channel: "어떤 semantic 채널이 활성화되어 있는가?"
        s_avg = self.avg_pool(seg_cond).squeeze(-1).squeeze(-1)   # (B, C_s)
        s_max = self.max_pool(seg_cond).squeeze(-1).squeeze(-1)   # (B, C_s)
        g_sem_c = torch.sigmoid(
            self.sem_channel_fc(s_avg) + self.sem_channel_fc(s_max))  # (B, C)
        g_sem_c = g_sem_c.unsqueeze(-1).unsqueeze(-1)     # (B, C, 1, 1)
        # Spatial: "어떤 위치에 semantic 신호가 존재하는가?"
        s_mean  = seg_cond.mean(dim=1, keepdim=True)      # (B, 1, H, W)
        s_max_s = seg_cond.amax(dim=1, keepdim=True)      # (B, 1, H, W)
        g_sem_s = torch.sigmoid(self.sem_spatial_conv(
            torch.cat([s_mean, s_max_s], dim=1)))         # (B, 1, H, W)
        g_sem = g_sem_c * g_sem_s                         # (B, C, H, W)

        # --- Step 2b: BEV Gate (from x) ---
        # Channel: "이 채널은 3D geometry 인코딩이 아닌가? (안전하게 modulate 가능한가?)"
        x_avg = self.avg_pool(x).squeeze(-1).squeeze(-1)          # (B, C)
        x_max = self.max_pool(x).squeeze(-1).squeeze(-1)          # (B, C)
        g_bev_c = torch.sigmoid(
            self.bev_channel_fc(x_avg) + self.bev_channel_fc(x_max))  # (B, C)
        g_bev_c = g_bev_c.unsqueeze(-1).unsqueeze(-1)     # (B, C, 1, 1)
        # Spatial: "이 위치의 geometry 정보를 건드려도 안전한가?"
        x_mean  = x.mean(dim=1, keepdim=True)             # (B, 1, H, W)
        x_max_s = x.amax(dim=1, keepdim=True)             # (B, 1, H, W)
        g_bev_s = torch.sigmoid(self.bev_spatial_conv(
            torch.cat([x_mean, x_max_s], dim=1)))         # (B, 1, H, W)
        g_bev = g_bev_c * g_bev_s                         # (B, C, H, W)

        # --- Step 3: Compose & Apply ---
        # gate opens only where semantic info exists AND geometry is safe
        gate = g_sem * g_bev                              # (B, C, H, W)
        out = x_norm * (1 + gate * gamma) + gate * beta   # (B, C, H, W)
        return out
...
class FDNResBlock(TimestepBlock):
    def __init__(
        ...
        self.norm_0 = GatedFDN(channels, seg_channels)
        self.norm_1 = GatedFDN(self.out_channels, seg_channels)
        
        # self.norm_0 = AdvancedGatedFDNv2(channels, seg_channels)
        # self.norm_1 = AdvancedGatedFDNv2(self.out_channels, seg_channels)
...
class LayoutDiffusionUNetModel(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                # if (i == 0 or i == 2) and level > 0:
                # if (i == 0 or i == 2) and not (level == 0 and i == num_res_blocks):
                if i == 0 or i == 2:
                    layers = [
                        FDNResBlock(
                            ch + ich,
                ...
                block = TimestepEmbedSequential(*layers)
                block.ctx_ds = ctx_ds  # use pre-upsample ds so SpatialTransformer gets matching context dim
    ...
    def forward(self, x, timesteps, img_metas, segmaps, obj_class=None, obj_bbox=None, obj_mask=None, is_valid_obj=None, obj_name=None, **kwargs):
        hs, extra_outputs = [], []
        ...
        seg_bev_maps = self.seg_aligner(segmaps, img_metas) # (B, C, H, W)
    ...
    def from_pretrained(self, pretrained_model_name_or_path, subfolder=None):
        ...
        try:
            ...
            if unexpected_keys:
                print(f'  Unexpected keys ({len(unexpected_keys)}): {sorted(unexpected_keys)}')
            if shape_mismatch_keys:
                print(f'  Shape mismatch keys ({len(shape_mismatch_keys)}):')
                for k in sorted(shape_mismatch_keys):
                    print(f'    {k}: model={self.state_dict()[k].shape}, ckpt={state_dict[k].shape}')
