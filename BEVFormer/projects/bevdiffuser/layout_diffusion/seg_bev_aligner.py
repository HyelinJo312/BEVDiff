...
#     Output: [B*V, C, H, W]  (full resolution, boundary-preserving)
#     """
#     def __init__(self, num_classes, embed_dim, out_channels):
#         super().__init__()
#         self.embed = nn.Embedding(num_classes + 1, embed_dim)

#         # 1×1: 공간 mixing 없는 채널 확장 → boundary perfectly sharp
#         # 3×3 → 1×1 으로 변경: FDN의 conv_gamma/conv_beta가 이미 3×3으로 local context 처리
...
#         # seg_emb = self.proj(seg_emb)                      # [B*V, out_channels, H, W]
#         return seg_emb
...
class SegEmbedEncoder(nn.Module):
    def __init__(self, num_classes, embed_dim, out_channels):
        ...
        self.embed = nn.Embedding(num_classes + 1, embed_dim)
        self.proj = nn.Conv2d(embed_dim, out_channels, kernel_size=1, bias=False)
    ...
    def forward(self, seg_id):
        ...
        # Nearest downsampling (이산 ID에 수학적으로 유효)
        seg_id_ds = F.interpolate(
            seg_id.unsqueeze(1).float(),
            scale_factor=0.5, mode='nearest', recompute_scale_factor=False
        ).squeeze(1).long()                           # [B*V, H//2, W//2]

        seg_emb = self.embed(seg_id_ds.long())        # [B*V, H//2, W//2, 64]
        seg_emb = seg_emb.permute(0, 3, 1, 2).contiguous()  # [B*V, 64, H//2, W//2]

        seg_emb = self.proj(seg_emb)                  # [B*V, 256, H//2, W//2]
        return seg_emb
...
class SegBEVAligner(nn.Module):
    ...
    def __init__(
        ...
        # ---- Segmentation embedding encoder (full resolution, 3×3 local context) ----
        ...
        # self.seg_encoder = nn.Embedding(num_classes + 1, emb_channels)
    ...
    def _apply_pre_ln(self, fmap):
        """Apply pre-LayerNorm to feature map (B*V, C, fH, fW)."""
        BV, C, fH, fW = fmap.shape
        t = fmap.permute(0, 2, 3, 1).reshape(-1, C)  # (BV*fH*fW, C)
        t = self.pre_ln(t)
        return t.view(BV, fH, fW, C).permute(0, 3, 1, 2).contiguous()
    ...
    def forward(self, seg_id, img_metas):
        ...
        # (4) Original pixel → normalised coords for grid_sample (no ida transform)
        ...
        v_feat = v / ds
        gx = 2.0 * (u_feat / (fW - 1.0)) - 1.0  # (B, V, QD)
