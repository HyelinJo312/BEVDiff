...
from .dino_bev_aligner import DINOBevEncoder, DINOBEVAligner
...
#     """
#     Single-scale SPADE fusion: Semantic BEV modulates DINO BEV per-pixel.

#     Semantic BEV generates spatially-adaptive gamma/beta to modulate
#     normalized DINO BEV features: out = GroupNorm(dino) * (1 + gamma) + beta.
#     """
#     def __init__(self, c_dino, c_seg, num_groups=32):
#         super().__init__()
#         self.param_free_norm = nn.GroupNorm(
#             num_groups=min(num_groups, c_dino),
#             num_channels=c_dino,
#             affine=False,
#         )
#         self.shared = nn.Sequential(
#             nn.Conv2d(c_seg, c_seg, kernel_size=3, padding=1),
#             nn.SiLU(),
#         )
#         self.conv_gamma = zero_module(nn.Conv2d(c_seg, c_dino, kernel_size=3, padding=1))
#         self.conv_beta = zero_module(nn.Conv2d(c_seg, c_dino, kernel_size=3, padding=1))
#         self.conv_gate  = nn.Conv2d(c_seg, c_dino, kernel_size=1)

#     def forward(self, dino_bev, seg_bev):
#         normalized = self.param_free_norm(dino_bev)
#         # seg_hidden = self.shared(seg_bev)
#         gamma = self.conv_gamma(seg_bev)
#         beta = self.conv_beta(seg_bev)
#         gate = torch.sigmoid(self.conv_gate(seg_bev))
#         out = normalized * (1 + gate * gamma) + gate * beta
#         return out

class DINOSemanticFusion(TimestepBlock):
    def __init__(
            self,
            channels,
            emb_channels,
    ...
    def forward(self, dino_bev, seg_bev):
        h = self.norm_0(dino_bev, seg_bev)
        h = self.in_layers(h)
        h = self.norm_1(h, seg_bev)
        h = self.out_layers(h)
        return self.skip_connection(dino_bev) + h
...
class LayoutDiffusionUNetModel(nn.Module):
    ...
    def __init__(
        ...
        # Semantic BEV fusion (optional)
        self.seg_aligner = SegBEVAligner(**seg_bev_aligner)
        self.dino_seg_fusion = DINOSemanticFusion(
                                    c_dino=self.dino_aligner.c_ctx,
                                    c_seg=seg_bev_aligner['emb_channels'],
                                )
        self.fused_bev_encoder = DINOBevEncoder(in_channels=self.dino_aligner.c_ctx)
        # self.seg_aligner = None
    ...
    def forward(self, x, timesteps, img_metas, dino_cond, seg_cond=None, obj_class=None, obj_bbox=None, obj_mask=None, is_valid_obj=None, obj_name=None, **kwargs):
        hs, extra_outputs = [], []
        ...
        # BEV context: DINO + optional Semantic fusion
        dino_bev_single = self.dino_aligner(
                            dino_cond['last_tokens'][:, 0, ...], dino_cond['patch_hw'],
                            img_metas, dino_cond['geom'])  
        seg_bev_single = self.seg_aligner(seg_cond, img_metas)                
        fused_bev = self.dino_seg_fusion(dino_bev_single, seg_bev_single)         # (B, c_ctx, bev_h, bev_w)
        ctx_bev_dict = self.fused_bev_encoder(fused_bev)        
        
        # ctx_bev_dict = self.dino_aligner(
        #     dino_cond['last_tokens'][:, 0, ...], patch_hw=dino_cond['patch_hw'],
        ...
        for module in self.input_blocks:
            dino_tokens = self._select_ctx(ctx_bev_dict, module).flatten(2).transpose(1, 2)
        ...
        # Middle block
        dino_tokens_mid = self._select_ctx(ctx_bev_dict, self.middle_block).flatten(2).transpose(1, 2)
