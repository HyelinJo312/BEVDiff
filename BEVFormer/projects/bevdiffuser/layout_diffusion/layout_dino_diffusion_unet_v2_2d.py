...
class LayoutDiffusionUNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, dino_cond, obj_class=None, obj_bbox=None, obj_mask=None, is_valid_obj=None, obj_name=None, **kwargs):
        ...
        for module in self.input_blocks:
            h, extra_output = module(h, emb, layout_outputs, dino_cond)  # dino_cond: 2D features
            if extra_output is not None:
                extra_outputs.append(extra_output)
            hs.append(h)
            
        # Middle block
        h, extra_output = self.middle_block(h, emb, layout_outputs, dino_cond)
        if extra_output is not None:
            extra_outputs.append(extra_output)
        # out_list.append(h)
        ...
        for i_out, module in enumerate(self.output_blocks):
            ...
            h, extra_output = module(h, emb, layout_outputs, dino_cond)
            if extra_output is not None:
                extra_outputs.append(extra_output)
            # if i_out in [1, 4]:
            #     out_list.append(h)
