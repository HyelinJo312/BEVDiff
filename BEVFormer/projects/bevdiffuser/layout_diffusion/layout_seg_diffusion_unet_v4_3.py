...
class AdvancedGatedFDN(nn.Module):
    """
    Spatial-Channel Decoupled Gating FDN (SCDG-FDN).

    Design
    ------
    dense gate G ∈ ℝ^{B×C×H×W} 를 두 signal의 rank-1 외적으로 분해:
      (1) Channel gate  g_c (B, C, 1, 1) : BEV feature x 로부터(SE-style) 산출.
          "어떤 채널을 semantic으로 받을지" — geometry 채널은 g_c[k]≈0 로 학습 가능.
      (2) Spatial gate  g_s (B, 1, H, W) : seg_cond 로부터 1x1 conv 로 산출.
          "어느 pixel에서 semantic 주입이 유용한지" — foreground 셀에서 열림.
    gate = g_c * g_s   →  broadcast (B, C, H, W), 메모리 낭비 없음.

    Residual-preserving modulation
    ------------------------------
        out = norm(x) * (1 + gate * γ) + gate * β
    gate → 0 일 때 out = norm(x) 로 identity 경로가 보장된다.
    γ, β 초기값을 0으로, gate bias 를 -2 로 두면 학습 시작 시 모듈은 GroupNorm과 동등 → 기존 backbone 의 stable reparameterization.
    """

    def __init__(self, norm_channels, cond_channels, se_reduction=16):
        super().__init__()
        self.param_free_norm = normalization(norm_channels)
        ...
        # (1) Channel gate — BEV feature x 에 대한 SE block
        hidden = max(norm_channels // se_reduction, 8)
        self.ch_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),                              # (B, Cn, 1, 1)
            nn.Conv2d(norm_channels, hidden, kernel_size=1),
            nn.SiLU(inplace=True),
            nn.Conv2d(hidden, norm_channels, kernel_size=1),      # pre-sigmoid logits
        )

        # (2) Spatial gate — seg_cond 에 대한 단일 1x1 conv
        self.sp_gate = nn.Conv2d(cond_channels, 1, kernel_size=1) # pre-sigmoid logits

        # Init: γ, β = 0, gate bias = -2 (sigmoid(-2) ≈ 0.12)
        # → 학습 시작 시 모듈은 param-free GroupNorm 과 동등
        # nn.init.zeros_(self.conv_gamma.weight); nn.init.zeros_(self.conv_gamma.bias)
        # nn.init.zeros_(self.conv_beta.weight);  nn.init.zeros_(self.conv_beta.bias)
        # nn.init.constant_(self.ch_gate[-1].bias, -2.0)
        # nn.init.constant_(self.sp_gate.bias, -2.0)

    def forward(self, x, seg_cond):
        # γ, β modulation parameter (B, Cn, H, W)
        normalized = self.param_free_norm(x)
        gamma = self.conv_gamma(seg_cond)
        beta  = self.conv_beta(seg_cond)

        # (1) channel gate : 공간적으로 일정한 per-channel mask
        #     GAP 이후 spatial 이 1x1 로 눌려 있어 "어느 채널을 열지" 만 학습.
        g_c = torch.sigmoid(self.ch_gate(x))                      # (B, Cn, 1, 1)

        # (2) spatial gate : 모든 채널에 공통인 per-pixel saliency
        #     seg_cond 가 비어 있는 BEV cell(도로/하늘/occluded) 에서 작아짐.
        g_s = torch.sigmoid(self.sp_gate(seg_cond))               # (B, 1, H, W)

        # rank-1 factorization : broadcast (B,Cn,1,1)*(B,1,H,W) → (B,Cn,H,W)
        # (channel, pixel) 쌍별 자유도가 사라져 geometry 채널의 우연한 덮어쓰기를
        # 구조적으로 억제한다 (inductive bias).
        gate = g_c * g_s                                          # (B, Cn, H, W)

        # residual-preserving modulation
        #   gate=0 → out = normalized (geometry 완전 보존)
        #   gate=1 → SPADE/FDN 과 동일한 full semantic modulation
        out = normalized * (1.0 + gate * gamma) + gate * beta
        return out
...
class FDNResBlock(TimestepBlock):
    def __init__(
        ...
        # --- ablation baseline (leave commented so we can toggle back) ---
        self.norm_0 = GatedFDN(channels, seg_channels)
        self.norm_1 = GatedFDN(self.out_channels, seg_channels)

        # # --- current: Spatial-Channel Decoupled Gating FDN ---
        # self.norm_0 = AdvancedGatedFDN(channels, seg_channels)
        # self.norm_1 = AdvancedGatedFDN(self.out_channels, seg_channels)
...
class LayoutDiffusionUNetModel(nn.Module):
    ...
    def __init__(
        ...
        # A1 (ControlNet-style): seg ds=1 map (seg_channels[0] ch) 을 입력 x 에 concat 하므로
        # 첫 conv 입력 채널을 in_channels + seg_channels[0] 로 확장. (forward 의 concat 과 짝)
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                # A1 (ControlNet-style): decoder 에 seg 를 주입하지 않음 → 모든 블록 plain ResBlock.
                # (seg 는 input conv 에서 한 번만 concat. 기존 FDNResBlock 분기 제거)
                layers = [
                    ResBlock(
                        ch + ich,
                        time_embed_dim,
    ...
    def enable_gradient_checkpointing(self):
        """
        Enable gradient checkpointing on all ResBlock / FDNResBlock / AttentionBlock modules.
        Called by train script when --gradient_checkpointing is set.
        """
        def _set(m):
            if isinstance(m, (ResBlock, FDNResBlock, AttentionBlock, ObjectAwareCrossAttention)):
                m.use_checkpoint = True
        self.apply(_set)

    def forward(self, x, timesteps, img_metas, seg_cond, obj_class=None, obj_bbox=None, obj_mask=None, is_valid_obj=None, obj_name=None, depth_maps=None, **kwargs):
        hs, extra_outputs = [], []
        ...
        seg_bev_map, seg_bev_prob = self.seg_aligner(seg_cond, img_metas, depth_maps=depth_maps) # (B, C, H, W)
        
        h = x.type(self.dtype)  # h: (B, C, H, W)
        for module in self.downsample_blocks:
            h = module(h)

        # --- A1 (ControlNet-style): seg ds=1 map 을 입력단에서 한 번만 concat ---
        # seg_bev_maps[1]: (B, seg_channels[0], 50, 50) — 입력 해상도(num_pre_downsample=0)와 정렬.
        # 첫 input conv 입력 채널을 in_channels + seg_channels[0] 로 확장해 둠.
        h = th.cat([h, seg_bev_map.type(h.dtype)], dim=1)  # (B, in_ch + seg_ch[0], 50, 50)
        ...
        # Decoder (A1: seg 주입 없음 — plain ResBlock decoder)
