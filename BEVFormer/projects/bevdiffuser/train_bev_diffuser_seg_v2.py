...
from layout_diffusion.layout_seg_diffusion_unet_v2 import LayoutDiffusionUNetModel, FDNResBlock
from scheduler_utils import DDIMGuidedScheduler
from model_utils import get_bev_model, build_unet
from test_bev_diffuser_seg import evaluate
...
def train():
    ...
    def get_condition(batch, drop_uncond=False):
        cond = {}
    ...
    def get_segmaps_cond(segmaps, drop_uncond=False):
        if drop_uncond:
        # if np.random.rand() < args.uncond_prob:
    ...
    if args.resume_from_checkpoint:
        ...
        bev_ckpt_path = os.path.join(resume_path, "bev_model.pth")
        if os.path.isfile(bev_ckpt_path):
            load_checkpoint(bev_model, bev_ckpt_path, map_location='cpu')
            logger.info(f"Loaded bev_model from {bev_ckpt_path}")
        else:
            logger.warning(f"bev_model checkpoint not found at {bev_ckpt_path}, skipping")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        # batch: ['img_metas', 'gt_bboxes_3d', 'gt_labels_3d', 'img', 'layout_obj_classes', 'layout_obj_bboxes', 'layout_obj_is_valid', 'layout_obj_names', 'default_obj_names', 'seg_maps']
        unet.train()
        train_loss = 0.0
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # Joint CFG drop: single Bernoulli per step → layout/seg/depth를 동시에 drop
                drop_uncond = bool(np.random.rand() < args.uncond_prob)
                # drop_uncond = False
                
                cond = get_condition(batch, drop_uncond=drop_uncond)

                # Segmentation maps [B, V, H, W] — present only when use_semantics=True
                seg_maps = torch.stack(batch['seg_maps'].data[0], dim=0)
                seg_cond = get_segmaps_cond(seg_maps, drop_uncond=drop_uncond)
                ...
                loss_dict = {
                    ...
                    # "train/drop_uncond": float(drop_uncond),
                }
