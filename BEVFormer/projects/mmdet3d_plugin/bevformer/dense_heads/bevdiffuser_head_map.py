...
class BEVDiffuserHead(BEVFormerHead):
    ...
    def __init__(self,
        ...
        self.multi_scale = MultiScaleConcat(in_chs=(256,512,1024), out_dim=256, mid=256, use_concat=True)
    ...
    def get_condition(self, kwargs, use_cond=True):
        ...
        if not use_cond:
            if isinstance(self.unet, LayoutDiffusionUNetModel):
                if 'obj_class' in self.unet.layout_encoder.used_condition_types:
                    cond['obj_class'] = torch.ones_like(cond['obj_class']).fill_(self.unet.layout_encoder.num_classes_for_layout_object - 1)
                    cond['obj_class'][:, 0] = self.unet.layout_encoder.num_classes_for_layout_object - 2
                if 'obj_name' in self.unet.layout_encoder.used_condition_types:
                    cond['obj_name'] = torch.stack(kwargs['default_obj_names'])
